...
{
    public partial class BlogController : AdminAuthorizeController
    {
        public virtual ActionResult Update(int? id)
        {
            ...
            if (id.HasValue)
            {
                Blog model = Blog.Select(id.Value, 2);
                model.UserSiteCategoryList = UserSite.SelectList(model.UserSiteID != null ? model.UserSiteID.Value : 0);
                model.CategoryList = BlogJoinBlogCategory.SelectList(id.Value);
                ViewBag.Title = "ویرایش خبر";
                return View(model);
            }
            else
            {
                ViewBag.Title = "اضافه کردن خبر";
        ...
        public virtual ActionResult Update(Blog obj)
        {
            if (ModelState.IsValid)
            {
                if (obj.ID != 0)
                {
                    try
                    {
                        ...
                        return RedirectToAction(MVC.Admin.Blog.Update());
                    }
                    catch
                    {
            ...
            obj.CategoryList = BlogJoinBlogCategory.SelectList(obj.ID);
            return View(obj);
        }
        ...
        public virtual JsonResult Select(int pageIndex, int pageSize, short? category)
        {
            ...
            result.Data = new
            {
                List = Blog.Select(pageIndex, pageSize, 2, out total, category),
                TotalRows = total
            };
