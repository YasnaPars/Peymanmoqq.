$(function () {
    ...
    $('.btnSaveNote').click(function () {
        if (sw1) {
            var note = {};
            note.Title = "یادداشت";
            note.Body = $(".content .paper").html();

            $.ajax({
                url: updateDynamicPartUrl,
                data: AddAntiForgeryToken({ obj: note }),
                type: 'POST',
                cache: false,
                dataType: "json",
                beforeSend: function () {
    ...
    try {
        Handlebars.registerHelper('ifZero', function (value) {
            if (value > 0) {
                return value;
            }
            else {
    ...
    function GetData() {
        $.ajax({
            url: selectUrl,
            data: AddAntiForgeryToken({ pageIndex: pageIndex, pageSize: pageSize, category: $.trim($('#ddlCategory').val()), id: recordId, status: recordStatus, type: $.trim($('#ddlType').val()) }),
            type: 'POST',
            cache: false,
            dataType: "json",
            beforeSend: function () {
            ...
            success: function (result) {
                ...
                try {
                    $('#grdData a.imgPreview').imgPreview({ containerID: 'imgPreviewWithStyles' });
                } catch (e) {
                }
    ...
    // اضافه کردن علامت ستاره کنار فیلدهای اجباری
    $('[data-val-required]').not('[type=hidden], [type=checkbox], [type=range], select#ddlCategory').after('<span class="required-indicator">*</span>');

    // فراخوانی بوت استرپ تولتیب
    $('[data-toggle="tooltip"]').tooltip();

    // دادن افکت به دراپ دوان
    try {
        $(".select2").select2({
            width: "100%",
            dir: "rtl",
    ...
    // دکمه انتخاب همه چک باکس ها
    try {
        $("#check-all, .icheck").iCheck({
            checkboxClass: "icheckbox_square-blue checkbox",
            radioClass: "iradio_square-blue"
        });

        $("#check-all").on("ifChanged", function () {
            var a = $("input.checkall:checkbox");
    ...
    // حذف یک رکورد
    $('.content').delegate('a.btnDeleteRow', 'click', function () {
        if (sw1) {
            if (confirm(deleteConfirmMsg)) {
                $this = $(this);
                var id = parseInt($.trim($this.data('id'))),
                    url = $.trim($this.data('url')),
                    photo = $.trim($this.data('photo'));

                $.ajax({
                    url: deleteUrl,
                    data: AddAntiForgeryToken({ id: id, url: url, photo: photo }),
                    type: 'POST',
                    cache: false,
                    dataType: "json",
                    beforeSend: function () {
                    ...
                    success: function (result) {
                        if (result) {
                            ...
                            alert('حذف اطلاعات با خطا مواجه شده است. لطفا مجددا تلاش نمایید');
                        }
    ...
    // درخواست ویرایش رکورد
    $('#grdData').delegate('a.btnEditRow', 'click', function () {
        if (sw1) {
            var id = parseInt($.trim($(this).data('id')));
    ...
    // کنسل کردن ویرایش رکورد
    $('#grdData').delegate('a.btnCancelRow', 'click', function () {
        $(this).closest('tr.display-none').fadeOut(function () {
            $(this).html(contentTD);
        });
    ...
    // ویرایش رکورد
    $('#grdData').delegate('form', 'submit', function (e) {
        $this = $(this);
        ...
        if ($this.valid()) {
            if (sw1) {
                $.ajax({
                    url: updateUrl,
                    data: AddAntiForgeryToken({ obj: $this.serializeObject() }),
                    type: 'POST',
                    cache: false,
                    dataType: "json",
                    beforeSend: function () {
                        $('.display-none', $this).fadeIn('fast');
                        sw1 = false;
                    },
                    success: function (result) {
                        $('.display-none', $this).fadeOut('fast');
                        sw1 = true;

                        if (result) {
                            pageSize = $.trim($('#ddlPageSize').val());
                            pageIndex = 1;
                            rowIndex = 1;
                            GetData();
                        }
                        else {
                            alert('ویرایش اطلاعات با خطا مواجه شده است. لطفا مجددا تلاش نمایید');
                        }
                    },
                    error: function () {
                        $('.display-none', $this).fadeOut('fast');
    ...
    // افزایش-کاهش اولویت یک رکورد
    $('#grdData').delegate('a.btnIncreaseRank, a.btnDecreaseRank', 'click', function () {
        if (sw2) {
            $.ajax({
                url: updateRankUrl,
                data: AddAntiForgeryToken({ id: parseInt($.trim($(this).data('id'))), type: $.trim($(this).data('type')), category: $.trim($('#ddlCategory').val()) }),
                type: 'POST',
                cache: false,
                dataType: "json",
                beforeSend: function () {
    ...
    // حذف دسته جمعی رکوردها
    $('.container-fluid').delegate('.btnDeleteAll', 'click', function () {
        if (sw2) {
            if (confirm(deleteAllConfirmMsg)) {
                $checks = $("input.checkall:checked");

                if ($checks.length <= 0) {
                    alert('حداقل یک رکورد را انتخاب نمایید');
                    return false;
                }
                ...
                $checks.each(function () {
                    ...
                    photoList.push($(this).data('photo'));
                });
                
                $.ajax({
                    url: deleteAllUrl,
                    data: AddAntiForgeryToken({ list: checkboxList, urlList: urlList, photoList: photoList }),
                    type: 'POST',
                    cache: false,
                    dataType: "json",
                    beforeSend: function () {
    ...
    // فراخوانی پنجره مودال
    $('.btnSelectImage').click(function () {
        $this = $(this);
        $("#modalSelectImage").on('show.bs.modal', function () {
            selectUrl = $this.data('href');
            GetData();
        }).modal({ backdrop: true });
    });

    $('#modalSelectImage').delegate('img.img-thumbnail', 'dblclick', function () {
        $("input#FileUploadID").val($(this).data('id'));
        $(".imgFileUpload").removeClass('display-none').attr('src', $(this).attr('src'));
        $("#modalSelectImage").modal("hide");
    });
    // پایان فراخوانی پنجره مودال
    ...
    try {
        var editor = CKEDITOR.replace('DynamicPage_Body', {
            contentsLangDirection: 'rtl',
            language: "fa",
            skin: "moonocolor",
            removeButtons: 'Save,NewPage,Templates'
    ...
    // پایان فراخوانی ckeditor

    // ستاره دار کردن رکورد
    $('body').delegate('a.btnStar', 'click', function () {
        if (sw1) {
            $this = $(this);
            var id = parseInt($.trim($this.data('id')));
            ...
            $.ajax({
                ...
                complete: function (xhr, status) {
                    ...
                    if (status === 'error' || !data) {
                        ...
                        $starList = $("a.btnStar[data-id=" + id + "]");

                        $starList.each(function () {
                            $iTag = $(":first-child", $(this));

                            if ($iTag.hasClass('active')) {
                                $iTag.removeClass('fa-star active').addClass('fa-star-o');
                            }
                            else {
                                $iTag.removeClass('fa-star-o').addClass('fa-star active');
                            }
                        });
                    }
    ...
    // انتقال ایمیل های حذف شده به صندوق پیام
    $('.mail-inbox').delegate('.btnMoveInbox', 'click', function () {
        if (sw1) {
            if (confirm(updateInboxConfirmMsg)) {
                $checks = $(".mails input:checked");
                ...
                $.ajax({
                    url: updateInboxUrl,
                    data: AddAntiForgeryToken({ list: checkboxList }),
                    type: 'POST',
                    cache: false,
                    dataType: "json",
                    beforeSend: function () {
    ...
    // نمایش جزئیات ایمیل
    $('.mail-inbox .mails .item').delegate('.bodyContent', 'click', function () {
        if (sw1) {
            $this = $(this);
            $grandParent = $this.closest('.mail-inbox');
            $parent = $this.closest('.item');
            var id = parseInt($.trim($this.data('id')));
            $nextDiv = $grandParent.fadeOut().next();
            $nextDiv.fadeIn(function () {
                $grandParent.find('.item').removeClass('active');
    ...
    // بازگشت به لیست ایمیل ها
    $('.message').delegate('.btnBackToMail', 'click', function () {
        $(this).closest('.message').fadeOut(function () {
            $(this).html(messageContent).prev('.mail-inbox').fadeIn();
        });
    ...
    // حذف یک کامنت
    $('.chat-content').delegate('.btnDeleteComment', 'click', function () {
        if (sw1) {
            if (confirm(deleteConfirmMsg)) {
                $this = $(this);
                var id = parseInt($.trim($this.data('id')));
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        if (result) {
                            $this.closest('.commnetRow').fadeOut(function () {
                                $(this).remove();
                            });
                        }
                        else {
    ...
    // تایید یا غیر فعال کردن یک کامنت
    $('.chat-content').delegate('.btnActiveComment', 'click', function () {
        if (sw1) {
            if (confirm(updateStatusConfirmMsg)) {
                $this = $(this);
                var id = parseInt($.trim($this.data('id'))),
                    type = $this.data('type');
                ...
                $.ajax({
                    url: updateStatusUrl,
                    data: AddAntiForgeryToken({ id: id, type: type }),
                    type: 'POST',
                    cache: false,
                    dataType: "json",
                    beforeSend: function () {
                    ...
                    success: function (result) {
                        if (result) {
                            $this.siblings('small').html($this.data('msg')).fadeIn();
                        }
                        else {
                            alert('ویرایش اطلاعات با خطا مواجه شده است. لطفا مجددا تلاش نمایید');
                        }
    ...
    // درخواست ویرایش کامنت
    $('.chat-content').delegate('.btnReplyComment', 'click', function () {
        if (sw1) {
            $this = $(this);
            var id = parseInt($.trim($this.data('id')));
            $nextElement = $this.closest('.commnetRow').find('.chat-in');
    ...
    // کنسل کردن ویرایش کامنت
    $('.chat-content').delegate('.btnCancelComment', 'click', function () {
        $(this).closest('.commnetRow').find('.chat-in').fadeOut(function () {
            $(this).html(contentElement)
        });
    ...
    // ویرایش کامنت
    $('#grdData .chat-wi').delegate('form', 'submit', function (e) {
        $this = $(this);
        $.validator.unobtrusive.parse($this);
        ...
        if (sw1) {
            if ($this.valid()) {
                $.ajax({
                    ...
                    success: function (result) {
                        if (result.status) {
                            $('.msgAlert', $this).html(result.msg).fadeIn();
                        }
                        else {
                            alert('ویرایش اطلاعات با خطا مواجه شده است. لطفا مجددا تلاش نمایید');
                        }
    ...
    try {
        ...
        $("div.rateit, span.rateit").bind('over', function (event, value) {
            $(this).attr('title', value);
        });

        $('div.rateit, span.rateit').on('beforerated', function (e, value) {
            if (!confirm('آیا مطمئن هستید که میخواهید امتیاز ' + value + ' ستاره را ثبت نمایید؟')) {
                e.preventDefault();
            }
        });

        $('div.rateit, span.rateit').on('beforereset', function (e) {
            if (!confirm('آیا مطمئن هستید که می خواهید امتیاز را ریست نمایید؟')) {
                e.preventDefault();
            }
        });
    } catch (e) {
    ...
    // پایان پلاگین افزودن تگ

    // دراپ دوان تغییر منوی اصلی
    $("#ParentID").on('change', function () {
        if (sw1) {
            var ddlSubMenu = $('#ddlSubMenu'),
                hdfSubMenuID = $('#HiddenSubMenuID');;

            $.ajax({
                url: menuUrl,
                data: AddAntiForgeryToken({ subMenuID: hdfSubMenuID.val(), parentID: parseInt($.trim($(this).val())) }),
                type: 'POST',
                cache: false,
                dataType: "json",
                beforeSend: function () {
                    ddlSubMenu.prev('.display-none').fadeIn('fast');
                    sw1 = false;
                },
                success: function (result) {
                    ddlSubMenu.prev('.display-none').fadeOut('fast');
                    ddlSubMenu.empty();
                    $(document.createElement('option')).attr('value', '0').text('اولین سطح').appendTo(ddlSubMenu);
                    $(result.List).each(function () {
                        $(document.createElement('option')).attr('value', this.Value).attr('selected', this.Selected).text(this.Text).appendTo(ddlSubMenu);
                    });
    ...
    $("#ddlSubMenu").on('change', function () {
        $('#HiddenSubMenuID').val($(this).val());
    });
    // پایان دراپ دوان تغییر منوی اصلی
}(jQuery));

// ویرایش رمز ایمیل های سایت
function ShowResult() {
    $(".form-horizontal.editPanel .loadingProgress").fadeOut(400);
    $(".form-horizontal.editPanel input[type=password]").val('');
}

function ClearForm() {
    $(".form-horizontal.editPanel #alertText").html('');
    $(".form-horizontal.editPanel .loadingProgress").fadeIn(100);
}
// پایان ویرایش رمز ایمیل های سایت

// نوار چسبان انتخاب همه
$(window).scroll(function () {
    if (($(window).width() <= 767 && $(this).scrollTop() > 300) || ($(window).width() > 767 && $(this).scrollTop() > 100)) {
        $('.mail-inbox .filters').addClass("stickyHeader");
    }
    else {
        $('.mail-inbox .filters').removeClass("stickyHeader");
    }
});
...
var App = function () {
    function a(a) {
        var b = $("#sidebar-collapse")[0],
            c = $("#cl-wrapper");
        $(".cl-sidebar");
        c.hasClass("sb-collapsed") ? ($(".fa", b).addClass("fa-chevron-right").removeClass("fa-chevron-left"), c.removeClass("sb-collapsed")) : ($(".fa", b).removeClass("fa-chevron-right").addClass("fa-chevron-left"), c.addClass("sb-collapsed"))
    }
